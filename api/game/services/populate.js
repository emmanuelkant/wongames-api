...
async function populate(params) {
  try {
    ...
    await createGamesDependencies(products);
    await createGames(products);
  } catch (error) {
    console.error('populate', handleError(error));
  }
};

async function createGamesDependencies(products) {
  const developers = {};
  const publishers = {};
  const categories = {};
  const platforms = {};
...
async function getGameInfo(slug) {
  try {
    ...
    console.error('getGameInfo', handleError(error));
  }
};
